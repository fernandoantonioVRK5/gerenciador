...
// 1. A importação do 'format' e 'parseISO' continua correta
...
export default function CardDetailPage() {
  ...
  const handleDelete = async (e, parcela) => {
    e.stopPropagation();
    if (!window.confirm("Isso excluirá TODAS as parcelas desta despesa. Você tem certeza?")) {
      return;
    }
    ...
    if (deleteError) {
      ...
      setTransactions(prev => 
        prev.filter(tx => (tx.despesas || tx.despesas_fixas).id !== parentExpense.id)
      );
    }
  };
